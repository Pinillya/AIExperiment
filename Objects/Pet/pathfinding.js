function FindPath(worldGrid, startPos, endPos)
{
	...
	function Neighbours (x, z) 
	{
		...
		if (northOpen)
		{
			result.push({x:x, z:north});
		}
		if (southOpen)
		{
			result.push({x:x, z:south});
		}
		if (westOpen)
		{
			result.push({x:west, z:z});
		}
		if (eastOpen)
		{
			result.push({x:east, z:z});
		}
	...
	{
	    var 
	    startNode = Node(null, {x:startPos.x, z:startPos.z}),
	    endNode   = Node(null, {x:endPos.x, z:endPos.z}),
